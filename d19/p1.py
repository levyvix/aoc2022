...
def max_geodes(blueprint, time_limit=24):
    ...
    while queue:
        minute, ore, clay, obsidian, geodes, ore_robots, clay_robots, obsidian_robots, geode_robots = queue.popleft()
        ...
        state = (minute, ore, clay, obsidian, ore_robots, clay_robots, obsidian_robots, geode_robots)
        if state in visited:
            continue
        visited.add(state)
        ...
        if ore >= blueprint['geode'][0] and obsidian >= blueprint['geode'][1]:
            actions.append((ore - blueprint['geode'][0],
                           clay,
        ...
        if ore >= blueprint['obsidian'][0] and clay >= blueprint['obsidian'][1]:
            if obsidian_robots < blueprint['geode'][1]:
                actions.append((ore - blueprint['obsidian'][0],
        ...
        if ore >= blueprint['clay']:
            if clay_robots < blueprint['obsidian'][1]:
                actions.append((ore - blueprint['clay'],
                               clay,
                               obsidian,
                               ore_robots,
                               clay_robots + 1,
        ...
        if ore >= blueprint['ore']:
            if ore_robots < max_ore_needed:
                actions.append((ore - blueprint['ore'],
                               clay,
                               obsidian,
                               ore_robots + 1,
                               clay_robots,
        ...
        actions.append((ore, clay, obsidian, ore_robots, clay_robots, obsidian_robots, geode_robots))

        # Process all actions
        for new_ore, new_clay, new_obsidian, new_ore_robots, new_clay_robots, new_obsidian_robots, new_geode_robots in actions:
...
def solve(input_data):
    ...
    for bp in blueprints:
        ...
        ic(f"Blueprint {bp['id']}: {geodes} geodes, quality {quality}")
        quality_sum += quality
